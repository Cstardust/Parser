...
class Dependency():
    ...
    def __str__(self):
        ...
        values = [
            str(self.idx), self.word, "_", "_", "_", "_",
    ...
    for i, d in enumerate(data):
        ...
        for item in d['dialog']:
            ...
            for word_idx, word in enumerate(utterance.split(' ')):
                head_word_idx, rel = rel_dct[turn].get(
                    word_idx + 1, [word_idx, 'adjct']
                )  # some word annoted missed, padded with last word and 'adjct'
                dep_lst.append(
                    Dependency(word_idx + 1, word, head_word_idx,
                               rel))  # start from 1
...
class DialogDataset(Dataset):
    ...
    def read_data(self, data_ids):
        ...
        for deps in tqdm(load_annoted(self.data_file, data_ids)):
            ...
            word_lst = []
            head_tokens = np.zeros(
                self.cfg.max_length, dtype=np.int64
            )  # same as root index is 0, constrainting by mask
            ...
            tokenized = self.cfg.tokenizer.encode_plus(
                word_lst,
                padding='max_length',
                truncation=True,
            ...
            sentence_word_idx = []
            for idx, (start,
                      end) in enumerate(tokenized.offset_mapping[0][1:]):
                if start == 0 and end != 0:
                    sentence_word_idx.append(idx)
            if len(sentence_word_idx) < self.cfg.max_length - 1:
                sentence_word_idx.extend(
                    [0] * (self.cfg.max_length - 1 - len(sentence_word_idx)))
    ...
    def __getitem__(self, idx):
        return self.inputs[idx], self.offsets[idx], self.heads[idx], self.rels[
            idx], self.masks[idx]
...
class ConllDataset(Dataset):
    ...
    def __init__(self, cfg, load_fn, train=True):
        self.train = train
        self.cfg = cfg
    ...
    def read_data(self):
        ...
        for deps in tqdm(self.load_fn(file, self.train)):
            ...
            for i, dep in enumerate(deps):
                ...
                if dep.head in ['_', '-1'
                                ] or int(dep.head) + 1 >= self.cfg.max_length:
                    head_tokens[i + 1] = 0
                    mask_tokens[i + 1] = 0
                else:
                    head_tokens[i + 1] = int(dep.head)
                ...
                if self.train:
                    # print(dep.id, dep.word, dep.head, dep.rel)
...
def load_inter(data_file):
    ...
    signal_iter = load_codt_signal(
        '/Users/mac/Documents/code/DialogDep-main/data/diag_test.conll',
        return_two=True)
    ...
    for d in data:
        ...
        for i, item in enumerate(d['dialog']):
            utterance = item['utterance']
            sent_lens_accum.append(sent_lens_accum[i] +
                                   len(utterance.split(' ')) + 1)
        sent_lens_accum[0] = 0
        ...
        for item in d['dialog']:
            ...
            tmp_signal = []
            for word_idx, word in enumerate(utterance.split(' ')):
                tail2head = rel_dct.get(turn,
                                        {1: [f'{turn}-{word_idx}', 'adjct']})
                head, rel = tail2head.get(
                    word_idx + 1, [f'{turn}-{word_idx}', 'adjct']
                )  # some word annoted missed, padded with last word and 'adjct'
                head_uttr_idx, head_word_idx = [
                    int(x) for x in head.split('-')
                ]
                ...
                if turn != head_uttr_idx:
                    dep_lst.append(
                        Dependency(
                            sent_lens_accum[turn] + word_idx + 1, word,
                            sent_lens_accum[head_uttr_idx] + head_word_idx,
                            rel))  # add with accumulated length
                else:
                    dep_lst.append(
                        Dependency(sent_lens_accum[turn] + word_idx + 1, word,
                                   -1, '_'))
...
class InterDataset(Dataset):
    ...
    def read_data(self):
        ...
        for idx, (deps, roles,
                  sign) in enumerate(load_inter(self.cfg.data_file)):
            seq_len = len(deps)
            signs.append(sign)
...
#                         sentence_word_idx[idx] = idx
            if len(sentence_word_idx) < 1024 - 1:
                sentence_word_idx.extend([0] *
                                         (1024 - 1 - len(sentence_word_idx)))
            offsets.append(torch.as_tensor(sentence_word_idx))
    ...
    def __getitem__(self, idx):
        return self.inputs[idx], self.offsets[idx], self.heads[idx], self.rels[
            idx], self.masks[idx], self.speakers[idx], self.signs[idx]
