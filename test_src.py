...
test_dataset = DialogDataset(CFG,
                             ...
                             data_ids=list(range(800)))  # 创建测试数据集
test_dataloader = DataLoader(test_dataset,
                             batch_size=CFG.batch_size)  # 创建测试数据集的数据加载器

model = DepParser(CFG)  # 创建依存解析器模型
print(model.load_state_dict(torch.load(f'./results/{CFG.plm.split("/")[-1].replace("-","_")}_base_par_new.pt')))
model = model.cuda()

arc_logits, rel_logits, similarities = torch.Tensor(), torch.Tensor(), torch.Tensor()  # 初始化存储变量
heads_whole, rels_whole, masks_whole = torch.Tensor(), torch.Tensor(), torch.Tensor()
for batch in test_dataloader:
    inputs, offsets, heads, rels, masks = batch  # 从批处理中获取输入、偏移量、头部、关系和掩码
    ...
    offsets, heads, rels, masks = to_cuda(data=(offsets, heads, rels,
                                                masks))  # 将数据移动到CUDA设备上

    with torch.no_grad():
        model.eval()
        arc_logit, rel_logit = model(inputs,
                                     offsets,
                                     heads,
                                     rels,
                                     masks,
    ...
    arc_logit[:, 0, 1:] = float('-inf')  # 将根节点到其他节点的弧概率设为负无穷
    arc_logit.diagonal(0, 1, 2)[1:].fill_(float('-inf'))  # 将对角线上的值设为负无穷

    arc_logits = torch.cat([arc_logits, arc_logit.cpu()])  # 拼接依存弧预测结果
    rel_logits = torch.cat([rel_logits, rel_logit.cpu()])  # 拼接依存关系预测结果

    heads_whole = torch.cat([heads_whole, heads.cpu()])  # 拼接真实头部
    rels_whole = torch.cat([rels_whole, rels.cpu()])  # 拼接真实关系
    masks_whole = torch.cat([masks_whole, masks.cpu()])  # 拼接掩码

origin4change = [rel2id[x]
                 for x in ['root', 'dfsubj', 'sasubj']]  # 获取需要改变的原始关系id列表

signal_dct = {}
for i, signals in enumerate(weak_signals):  # 遍历弱信号列表
    for s in signals:
        signal_dct[s] = weak_labels[i]  # 构建信号字典
print(signal_dct)

rel_preds = rel_logits.argmax(-1)  # 计算关系预测结果
head_preds = arc_logits.argmax(-1)  # 计算头部预测结果

# 读取评估集标签并对齐模型输出存储在
# heads_new_whole, rels_new_whole, signals_new_whole 三个张量中用于后续计算
max_len = CFG.max_length
signals_new_whole = torch.Tensor()
heads_new_whole, rels_new_whole = torch.Tensor(), torch.Tensor()
for sample_idx, (deps, pred_signals) in tqdm(
        enumerate(
            zip(
                load_annoted(CFG.data_file),
                load_codt_signal(
                    './data/diag_test.conll'
                )))):
    seq_len = len(deps)
    if seq_len == 0:
        continue
    ...
    heads, rels = torch.full(size=(max_len, ),
                             fill_value=-2).int(), torch.zeros(max_len).int()
    ...
    for idx, head in enumerate(heads[1:]):
        ...
        if ((len(splits) > 2 and
             (head < splits[cnt] or head >= splits[cnt + 1])) or idx - head
                > 0) and rels[idx + 1] in origin4change:  # cross 'edu'

            rels[idx + 1] = signals[idx + 1]

            if rels[idx + 1] in [
                    rel2id['cond']
            ] or (not attr and rels[idx + 1] == rel2id['attr']):  # reverse
                ...
                if len(tail_idx) == 0:  # ring or fail
                    ...
                    head_idx = (
                        heads == idx +
                        1).nonzero() if head_idx == tail_idx else head_idx
                elif len(head_idx) != 0:
                    heads[tail_idx[0]] = 0
                    heads[head_idx[0]] = tail_idx[0]

            # special cases
            if word_lst[idx + 1] == '好' and word_lst[idx] in ['你', '您'
                                                              ]:  # reverse
                tmp_heads = heads.clone()
                tmp_heads[:splits[cnt + 1]] = 0
                tail_idx = (tmp_heads == idx + 1).nonzero()  # find tail index
...
# 计算Inner依赖关系准确率
arc_logits_correct = (heads_new_whole == heads_whole).long() * masks_whole * (
    rels_whole >= 21).long()
...
print(rel_logits_correct.sum() / (masks_whole *
                                  (rels_whole < 21).long()).sum())
print(arc_logits_correct.sum() / (masks_whole *
                                  (rels_whole < 21).long()).sum())
...
cnt = 0
inter_heads_whole, inter_rels_whole, inter_masks_whole = torch.Tensor(
...
arc_logits_correct_inner = (head_preds == heads_whole).long() * masks_whole * (
    rels_whole >= 21).long()
arc_logits_correct_inter = (inter_heads_preds
                            == inter_heads_whole).long() * inter_masks_whole
inter_edu_uas = (
    arc_logits_correct_inner.sum() + arc_logits_correct_inter.sum()) / (
        (rels_whole >= 21).long().sum() + inter_masks_whole.long().sum())

rel_logits_correct_inner = (rel_preds
                            == rels_whole).long() * arc_logits_correct_inner
rel_logits_correct_inter = (
    inter_rels_preds == inter_rels_whole).long() * arc_logits_correct_inter
inter_edu_las = (
    rel_logits_correct_inner.sum() + rel_logits_correct_inter.sum()) / (
        (rels_whole >= 21).long().sum() + inter_masks_whole.long().sum())
